...
class ContractChangeForm extends Model
{
    public $contract_id;
    public $type;
    public $remark;
    ...
    public function validateContractId()
    {
        ...
        if(!($this->contract->status == Contract::STATUS_MODIFY && $this->contract->invalid_status == Contract::MODIFY_PENDING_REVIEW))
        {
            $this->addError('contract_id','合同状态必须是变更待审核状态');
        }
    }
    ...
    public function save()
    {
        ...
        if($this->type == 1)
        {
            ...
            ContractRecord::CreateRecord($this->contract->id,$admin->name.'通过了合同变更审批，审批备注为：'.$this->remark,$admin);
            return true;
        }
        else if($this->type == 2)
        {
            $this->contract->status = Contract::STATUS_MODIFY;
            $this->contract->correct_status = Contract::MODIFY_NO_REVIEW;
            $this->contract->save(false);
            ContractRecord::CreateRecord($this->contract->id,$admin->name.'驳回了合同变更审批，审批备注为：'.$this->remark,$admin);
            return true;
        }
    }
}
