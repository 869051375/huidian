...
class BatchCalculateProfitForm extends Model
{
    public $rate;
    public $point;
    public $fix_point_id;
    ...
    public function validateVirtualOrderId()
    {
        ...
        {
            $this->addError('virtual_order_id','找不到指定的虚拟订单');
        }
        else
        {
...
//            }
    ...
    {
        $this->order_ids = explode(',',rtrim($this->order_id,','));
        $this->orders = Order::find()->where(['in','id',$this->order_ids])->all();
            ...
            foreach ($this->orders as $order)
            {
                ...
                if($order->getExpectedCost() != $order->getCost())
                {
                    ...
                    $data[]= [
                        ...
                        'type' => ExpectedProfitSettlementDetail::TYPE_CORRECT,
                        'company_id' => $order->salesmanCompany ? $order->salesmanCompany->id : 0,
                        'company_name' => $order->salesmanCompany ? $order->salesmanCompany->name : '',
                        'title' => '预计利润计算',
                        'remark' => '',
    ...
    public function save()
    {
        ...
        $recordModels = PerformanceRecord::find()->where(['in','order_id',$this->order_ids])->all();
        foreach($recordModels as $record)
        {
            if(floatval($record->lavePerformance()) == 0) continue;//如果没有业绩跳出
            $rate = 100;//负责人的百分比
            $partition_rate = 0;//提成比例
//            $date = $this->getYearMonth();
            ...
            if($record->order->orderTeams)
            {
                foreach($record->order->orderTeams as $team)
                {
                    ...
                    $data[] = [
                        ...
                        'reward_proportion' => $team_partition_rate,//提成比例
                        'creator_id' => $admin->id,
                        'creator_name' => $admin->name,
                        'created_at' => $time,
                    ];
            ...
            /** @var Order $order_save */
            $order_save = Order::find()->where(['id'=>$record->order_id])->one();
            $order_save->is_apply = 0;
            $order_save->actual_profit_calculate = 1;
            $order_save->save(false);
            /** @var OrderBalanceRecord $order_balance_record */
            $order_balance_record = OrderBalanceRecord::find() -> where(['order_id' =>$order_save->id]) -> orderBy(['id'=>SORT_DESC]) -> one();
