...
<ul class="nav nav-tabs">
    <?php if (Yii::$app->user->can('product/update')): ?>
        <li<?php if (Yii::$app->controller->action->id == 'package-update'): ?> class="active"<?php endif; ?>>
            <a href="<?= \yii\helpers\Url::to(['product/package-update', 'id' => $product->id]) ?>">套餐基本信息设置</a>
        </li>
    <?php endif; ?>

    <?php if (Yii::$app->user->can('product/update')): ?>
        <li<?php if (Yii::$app->controller->action->uniqueId == 'package-product/list'): ?> class="active"<?php endif; ?>>
            <a href="<?= \yii\helpers\Url::to(['package-product/list', 'id' => $product->id]) ?>">包含商品设置</a>
        </li>
    <?php endif; ?>

    <?php if (Yii::$app->user->can('product-price/list')): ?>
        <li<?php if (Yii::$app->controller->action->id == 'price'): ?> class="active"<?php endif; ?>>
            <a href="<?= \yii\helpers\Url::to(['product/price', 'product_id' => $product->id]) ?>">套餐商品价格</a>
        </li>
    <?php endif; ?>
    <!--
        ...
        || Yii::$app->controller->action->uniqueId == 'package-related/list'): ?> class="active"<?php endif; ?>>
