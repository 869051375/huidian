    ...
    foreach ($models as $model):
        ...
        <tr>
            <td>
                <?= $model->name?><br/>
                <?= $model->phone?><br />
                服务中单数：<?= isset($model->customerService->servicing_number) ? $model->customerService->servicing_number : 0 ?>
            </td>
            <td><?= $model->email?></td>
            <td>
                <?php if ($model->image):?>
            ...
            <td class="text-right">
                ...
                <?php elseif($actionUniqueId == 'administrator/list-customer-service'):?>
                    <?php if (Yii::$app->user->can('administrator/update-customer-service')): ?>
                        <a class="btn btn-xs btn-white" href="<?= Yii::$app->urlManager->createUrl(['/administrator/customer-service-update', 'id' => isset($model->customerService->id) ? $model->customerService->id : 0]) ?>">编辑客服</a>
                        <a class="btn btn-xs btn-white"
